...
def scrapePageSMI(link, earl):
    ...
    rows = dom.cssselect("#content > div > div > table tr")[2:] # remove first two rows
    
    for row in rows:
        try:
            date=datetime.strptime(row.cssselect("td > b > font")[0].text , "%m/%d/%Y")
            # date = row.cssselect("td > b > font")[0].text
            title=row.cssselect("td a")[0].text
            link='http://mid.ru' + row.cssselect("td a")[0].get('href')
            # print date, title,link
            scraperwiki.sql.save(unique_keys, {'link':link,  'date':date, 'title':title})
        except:
            print 'ой, не тот ряд: ', row.cssselect("td > b > font")[0].text
    
    # print date
    # теперь выясним, не стоит ли остановиться и
    if date < earl:
...
link = 'http://mid.ru/bdomp/brp_4.nsf/pressotofpr!OpenView&Start=1' #стартовая позиция        
while True:
